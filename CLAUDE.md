...
- Paralell population plan- I want you to follow  Reception/Kindergarten Worksheet Population Plan but plan is in a way that I am able to generate all subtopics for reception/kindergarten except counting-to-10 and number-recognition as they have already been done. Before doing anything, I want you to make at least 3 agents work in paralell. each agent is following the population plan individually. Each agent should analyse the existing prompt for the assigned configuration and research the best quality worksheets/printables related to the subtopic to improve and refine the existing prompt, make sure that prompt produces a response which is compatible with interactive mode. and then follow the plan.ask if you have any question.