...
const WORKSHEET_SLUG = 'number-counting-number-bonds'
const WORKSHEET_ANSWERS = ["2", "1", "B", "6", "3"]
...
test.describe(`Interactive: Number Bonds Mixed Layout`, () => {
  test('should complete with 100% score', async ({ page }) => {
    ...
    await page.waitForTimeout(2000)
    const scoreElement = page.locator('.fixed.inset-0 >> text=/\\d+%/').first().or(page.locator('text=/\\d+%/').first())
